...
const createSong=asyncHandler(async(req,res)=>{
    try {
        ...
        {
            const coverImageResult=await uploadToCloudinary(req.files.coverImage[0].path,"spotify/covers")
...
const updateSong=asyncHandler(async(req,res)=>{
    try {
        ...
        if(song)
        {
            ...
            if(req.files &&req.files.audio)
            {
                const result=await uploadToCloudinary(req.files.audio[0].path,"spotify/songs")
            ...
            if(req.files &&req.files.coverImage)
            {
                const result=await uploadToCloudinary(req.files.coverImage[0].path,"spotify/covers")
...
const getTopSong=asyncHandler(async(req,res)=>{
    try {
        ...
        const song=await Song.find().sort({plays:-1}).limit(parseInt(limit))
        .populate("artist","name image")
        .populate("album","title coverImage")
        if(song.length>0)
        {
...
const getNewSong=asyncHandler(async(req,res)=>{
    try {
        const {limit=10}=req.query
        const song=await Song.find().sort({createdAt:-1}).limit(parseInt(limit))
        .populate("artist","name image")
        .populate("album","title coverImage")
        if(song.length>0)
        {
